...
// 创建一个用于软件 I2C 的 TwoWire 实例
TwoWire WireOLED = TwoWire(1);  // 使用 Wire1 的资源，但引脚由我们指定

// 修改：使用自定义的 WireOLED，并指定复位引脚（-1 表示无复位脚）
...
{
   // 初始化软件 I2C：SDA = GPIO4, SCL = GPIO5
  WireOLED.begin(4, 5);  // SDA, SCL

  // 初始化 OLED
  if (!display.begin(SSD1306_SWITCHCAPVCC, 0x3C)) { // 地址通常为 0x3C 或 0x3D
